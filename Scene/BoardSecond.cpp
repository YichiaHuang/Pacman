...
void BoardSecond::Initialize() {
    ...
    btn->SetOnClickCallback(std::bind(&BoardSecond::BackOnClick, this, 1));
    AddNewControlObject(btn);
    ...
    btn->SetOnClickCallback(std::bind(&BoardSecond::PrevPage, this));
    AddNewControlObject(btn);
    AddNewObject(new Engine::Label("Prev", "prstartk.ttf", 48, halfW-450, halfH * 7 / 4, 0, 0, 0, 255, 0.5, 0.5));

    btn = new Engine::ImageButton( "stage-select/floor.png", "stage-select/bott.png", halfW - 200+450, halfH * 7 / 4 - 50, 400, 100);
    btn->SetOnClickCallback(std::bind(&BoardSecond::NextPage, this));
    AddNewControlObject(btn);
    ...
    btn->SetOnClickCallback(std::bind(&BoardSecond::ChangeOnClick, this, 1));
    AddNewControlObject(btn);
    AddNewObject(new Engine::Label("Stage2", "prstartk.ttf", 48, halfW+450, halfH * 1 / 4+30, 0, 0, 0, 255, 0.5, 0.5));
}
void BoardSecond::Terminate() {
    IScene::Terminate();
}

void BoardSecond::BackOnClick(int stage) {
    currentPage = 0; 
    sortedScores.clear();
...
void BoardSecond::ChangeOnClick(int stage){
    currentPage = 0; 
    sortedScores.clear();
...
std::vector<std::tuple<std::string, int, std::string>> BoardSecond::LoadAndSortScores() {
    std::vector<std::tuple<std::string, int, std::string>> scores;
    std::ifstream scoreFile("Resource/score_2.txt");
...
void BoardSecond::DisplayScores() {
    for (auto& label : scoreLabels) {
        RemoveObject(label->GetObjectIterator());
    }
    scoreLabels.clear();
