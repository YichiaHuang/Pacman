...
#include "Enemy/SuppressorEnemy.hpp"
    ...
    for (int i = 0; i < SpeedMult; i++) {
        ...
        switch (current.first) {
            ...
            case 3:
                ...
                break;
            case 4:
                EnemyGroup->AddNewObject(enemy = new SuppressorEnemy(SpawnCoordinate.x, SpawnCoordinate.y));
                break;
            default:
                continue;
        }
...
void PlayScene::OnMouseUp(int button, int mx, int my) {
    ...
    if (button & 1) {
        if (mapState[y][x] != TILE_OCCUPIED) {
            if (!preview){
                Engine::LOG(Engine::INFO) << "No preview object exists.";
                return;
            }
            // Check if valid.
            if (!CheckSpaceValid(x, y)) {
                Engine::LOG(Engine::INFO) << "CheckSpaceValid failed at (" << x << ", " << y << ")";
...
void PlayScene::ConstructUI() {
    ...
    // Button 3: SniperTurret
    btn = new TurretButton("play/floor.png", "play/dirt.png",
        Engine::Sprite("play/tower-base.png", 1446, 136, 0, 0, 0, 0),
        Engine::Sprite("play/turret-3.png", 1446, 136 - 8, 0, 0, 0, 0),
...
void PlayScene::UIBtnClicked(int id) {
    ...
    if (id == 0 && money >= MachineGunTurret::Price)
        ...
        next_preview = new SniperTurret(0, 0);
...
bool PlayScene::CheckSpaceValid(int x, int y) {
    ...
    mapState[y][x] = map00;
    if (map[0][0] == -1){
        Engine::LOG(Engine::INFO) << "No path from start to end!";
        return false;
    }
    for (auto &it : EnemyGroup->GetObjects()) {
        Engine::Point pnt;
        ...
        if (map[pnt.y][pnt.x] == -1){
            Engine::LOG(Engine::INFO) << "Current enemy at (" << pnt.x << "," << pnt.y << ") has no path.";
            return false;
        }
    }
