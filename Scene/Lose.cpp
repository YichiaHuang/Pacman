...
void Lose::Initialize() {
    ...
    btn->SetOnClickCallback(std::bind(&Lose::ReplayOnClick, this));
    AddNewControlObject(btn);
    AddNewObject(new Engine::Label("Replay", "prstartk.ttf", 48, halfW - 250, halfH * 7 / 4, 0, 0, 0, 255, 0.5, 0.5));

    // Back Button - 右邊
    btn = new Engine::ImageButton("win/dirt.png", "win/floor.png", halfW + 250 - 200, halfH * 7 / 4 - 50, 400, 100);
    btn->SetOnClickCallback(std::bind(&Lose::BackOnClick, this, 2));
    AddNewControlObject(btn);
...
void Lose::BackOnClick(int stage) {
    // Change to select scene.
    Engine::GameEngine::GetInstance().ChangeScene("stage-select");
}

void Lose::ReplayOnClick() {
